...
Filename: shortestPathTest.cpp
Description:  This program creates a graph by inserting 1 - 10000 into a graph
in random order.  It then tests how long it takes to build a path between two 
nodes from 1000-10000 times.
*/
...
int main() {
  ...
  for(int i = 100; i <= 1000; ) {
    ...
    for(int j = 0; j < i*2; j+=2) g.shortestPath(values[j], values[j+1]);
